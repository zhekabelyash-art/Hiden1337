...
w.BorderSizePixel=0
w.Visible=true -- Показываем сразу
w.Parent=g
...
function Sec(t)
	...
	local bgCr=Instance.new("UICorner");bgCr.CornerRadius=UDim.new(0,4);bgCr.Parent=bg
...
function Tgl(nm,dsc,def,cb)
	...
	local rCr=Instance.new("UICorner");rCr.CornerRadius=UDim.new(0,5);rCr.Parent=r
	...
	local swCr=Instance.new("UICorner");swCr.CornerRadius=UDim.new(1,0);swCr.Parent=sw
	...
	local knCr=Instance.new("UICorner");knCr.CornerRadius=UDim.new(1,0);knCr.Parent=kn
	local st=Instance.new("TextLabel");st.Size=UDim2.new(0,40,0,9);st.Position=UDim2.new(1,-44,0.5,9); -- ИСПРАВЛЕНО!
	st.BackgroundTransparency=1;st.Text=def and "ON" or "OFF";st.Font=Enum.Font.GothamBold;st.TextSize=7;st.TextColor3=def and Color3.fromRGB(65,185,105) or Color3.fromRGB(115,115,130);st.Parent=r
	local cr=def
	function Up(v)
		cr=v;TS:Create(sw,TweenInfo.new(0.15),{BackgroundColor3=v and Color3.fromRGB(65,185,105) or Color3.fromRGB(48,48,60)}):Play();TS:Create(kn,TweenInfo.new(0.15),{Position=v and UDim2.new(1,-16,0.5,-6) or UDim2.new(0,3,0.5,-6)}):Play();st.Text=v and "ON" or "OFF";st.TextColor3=v and Color3.fromRGB(65,185,105) or Color3.fromRGB(115,115,130);if cb then cb(v)end
	end
